...
export default {
  ...
  methods: {
    ...
    async getTableList() {
      ...
      try {
        let res = await qualityCheckRisklist(params);
        if (res.data.status === "0") {
    ...
    deleteItem(item) {
      this.$confirm("此操作将永久删除该项, 是否继续?", "提示", {
        ...
        .then(() => {
          ...
          qualityCheckRiskcheckUpdate(params).then((res) => {
            if (res.data.status === "0") {
