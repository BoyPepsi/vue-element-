  ...
  <div id="lift-detail">
    <div class="item-select">
      ...
      <el-row class="row-item" v-if="code !== '0402020305'">
        <span>塔吊编号:</span>
        {{addCheckForm.liftNum | accordingIdFindName(filterList)}}
      </el-row>
      <el-row class="row-item">
        <span>型号:</span>
      ...
      <el-row class="row-item" v-if="code == '0402020307'">
        <span>进场日期:</span>
        {{addCheckForm.joinTime | accordingIdFindName(filterList) | transformDate}}
      </el-row>
      <el-row class="row-item" v-if="code == '0402020307'">
        <span>出场日期:</span>
        {{addCheckForm.outTime | accordingIdFindName(filterList) | transformDate}}
      </el-row>
      <el-row class="row-item" v-if="code =='0402020301'">
        <span>加节顶升高度范围:</span>
        {{addCheckForm.hightArea | accordingIdFindName(filterList)}}
      </el-row>
      <el-row class="row-item" v-if="code =='0402020301'">
        <span>第几道扶墙杆:</span>
        {{addCheckForm.wallRod | accordingIdFindName(filterList)}}
      </el-row>
      <el-row class="row-item" v-if="code == '0402020307'">
        <span>合格证:</span>
        <img
          v-for="item in addCheckForm.fileList"
          :key="item.uid"
          :src="item.url"
          width="200px"
          height="auto"
...
<script>
import { maintableList, maintableReview,qualityCheckEditQuestionBimStatus } from "@/http/api.js";
