<template>
  <div id="sysdictionariesList">
    ...
    <div class="brad-nav">
      ...
      <div class="right-button">
        <el-button
          ...
          v-permission="{ action: 'admin:syscheck:save' }"
        >添 加</el-button>
      </div>
    </div>
    <div class="main">
      ...
      >
        <el-table-column prop="sort" label="排序列" align="center" min-width="15%"></el-table-column>
        <!-- <el-table-column prop="parentid" label="父级ID" min-width="10%" align="center"></el-table-column> -->
        <el-table-column prop="name" label="名称" align="center" min-width="10%"></el-table-column>
        <el-table-column prop="remarks" label="备注" align="center" min-width="10%"></el-table-column>
        <el-table-column prop="level" label="层级" min-width="10%" align="center"></el-table-column>
        ...
        <el-table-column fixed="right" label="操作" min-width="25%" align="center">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              v-permission="{ action: 'admin:syscheck:tree' }"
            >下一级</el-button>
            <el-button
              icon="el-icon-edit"
              type="text"
              ...
              v-permission="{ action: 'admin:syscheck:update' }"
            >编辑</el-button>
            <el-button
              type="text"
              icon="el-icon-delete"
              ...
              class="red"
              size="small"
              v-permission="{ action: 'admin:syscheck:delete' }"
            >删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 编辑-新增-弹窗 -->
    <el-dialog :title="dialogTitle" :visible.sync="showModel" width="30%" center>
      <div class="mainBox">
        <span style="margin-right: 20px">字典名称:</span>
        <el-input v-model="roleName" placeholder="请输入字典名字:" width="200px" maxlength="50"></el-input>
      </div>
      <div class="mainBox">
        <span style="margin-right: 20px">备注内容:</span>
        <el-input v-model="remarks" placeholder="请输入备注:" width="200px" maxlength="100"></el-input>
      </div>
      <span slot="footer" class="dialog-footer">
        <el-button @click="cancel" size="mini">取 消</el-button>
        <el-button type="primary" @click="addDictionary" size="mini" v-if="flag">新 增</el-button>
    ...
    <el-dialog title="添加子级" :visible.sync="showChild" width="30%" center>
      <div class="mainBox">
        <span style="margin-right: 20px">字典名称:</span>
        <el-input v-model="child" placeholder="请输入字典名字:" width="200px" maxlength="50"></el-input>
      </div>
      <div class="mainBox">
        <span style="margin-right: 20px">备注内容:</span>
        <el-input v-model="remarks" placeholder="请输入备注:" width="200px" maxlength="100"></el-input>
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="addChilds" size="mini">添加子级</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  created() {
    ...
    this.parentid = "251478522642563072";
    this.getSyscheckListFun(this.parentid);
  ...
  methods: {
     ...
     * date:2020-6-9
     */
    async getSyscheckListFun(parentid) {
      let params = {
      ...
      try {
        // let res = await getSyscheckList(params);
        let res = await syscheckListNothing(params)
    ...
    clickBradItem(item, index) {
      ...
      this.getSyscheckListFun(item.id);
    },
    /**
    ...
    async addChilds() {
      if (this.child.length === 0) {
        this.$message({ type: "error", message: "有信息未输入" });
      } else {
        let chidlParams = {
          name: this.child,
        ...
        try {
          let res = await saveSyscheck(chidlParams);
          if (res.data.status === "0") {
            this.$message({
              type: "success",
              message: "操作成功, 成功添加子级!",
    ...
    midEdit(row) {
      ...
      this.showModel = true;
      this.flag = false;
      this.roleName = JSON.parse(JSON.stringify(row.name));
      this.remarks = JSON.parse(JSON.stringify(row.remarks));
    ...
    async addDictionary() {
      ...
      try {
        let res = await saveSyscheck(params);
        if (res.data.status === "0") {
    ...
    async modifyRole() {
      ...
      try {
        let res = await updateSyscheck(params);
        if (res.data.status === "0") {
          this.$message({
            type: "success",
            message: "操作成功, 信息修改成功!",
