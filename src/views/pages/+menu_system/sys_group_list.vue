    ...
    <div class="brad-nav">
      ...
      <div class="right-button">
        <el-button
          ...
          v-permission="{ action: 'admin:sysgroup:save' }"
        >添 加</el-button>
      </div>
    </div>
    <div class="main">
        ...
        <el-table-column fixed="right" label="操作" min-width="25%" align="center">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              v-permission="{ action: 'admin:sysgroup:update' }"
            >编辑</el-button>
            <el-button
              type="text"
              icon="el-icon-delete"
              ...
              class="red"
              v-permission="{ action: 'admin:sysgroup:delete' }"
            >删除</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  created() {
    ...
    if (userInfo.isAdmin === 1) {
      this.parentid = "bc7b99932b23464fa0917b147ab62dc6";
    } else {
      if (
    ...
    this.getSysgroupListFun(this.parentid);
  ...
  methods: {
     ...
     * date:2020-6-9
     */
    async getSysgroupListFun(parentid) {
      let params = {
      ...
      try {
        // let res = await getSysgroupList(params);
        let res = await sysgroupListNothing(params);
    ...
    clickBradItem(item, index) {
      ...
      this.getSysgroupListFun(item.id);
    },
    /**
    ...
    async addChilds() {
      if (this.child.length === 0) {
        ...
        try {
          let res = await saveSysgroup(chidlParams);
          if (res.data.status === "0") {
            this.$message({
              type: "success",
              message: "操作成功, 成功添加子级!",
    ...
    async addDictionary() {
      ...
      try {
        let res = await saveSysgroup(params);
        if (res.data.status === "0") {
    ...
    async modifyRole() {
      ...
      try {
        let res = await updateSysgroup(params);
        if (res.data.status === "0") {
          this.$message({
            type: "success",
            message: "操作成功, 信息修改成功!",
