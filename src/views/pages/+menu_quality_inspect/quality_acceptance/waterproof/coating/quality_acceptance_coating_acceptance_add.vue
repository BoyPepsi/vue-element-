      ...
      >
        <el-row class="row-item">
          <el-form-item label="工程名称:">
            <el-input disabled v-model="projectName" style="width:30%"></el-input>
          </el-form-item>
        </el-row>
        <el-row class="row-item">
          <el-form-item label="施工单位:" prop="bidUnitIdToName">
            <el-input disabled v-model="addFromData.bidUnitIdToName" style="width:30%"></el-input>
          </el-form-item>
        </el-row>
        <el-row class="row-item">
          <el-form-item label="施工部位:" prop="floor">
            ...
            <el-select
              ...
              placeholder
              style="width:10%;margin-left:7px"
              @change="getFool"
              disabled
            >
              <el-option
        ...
        <el-row class="row-item">
          <el-form-item label="具体位置:">
            <el-select v-model="addFromData.seatCode" placeholder="请选择" style="width:30%" disabled>
              <el-option
        ...
        <el-row class="row-item">
          <el-form-item label="防水涂料品牌:">
            <el-input v-model="addFromData.brand" style="width:30%" disabled></el-input>
          </el-form-item>
        </el-row>
        <el-row class="row-item">
          <el-form-item label="防水涂料型号:">
            <el-input v-model="addFromData.model" style="width:30%" disabled></el-input>
          </el-form-item>
        </el-row>
        <el-row class="row-item">
          <el-form-item label="涂料设计厚度(mm):">
            <el-input v-model="addFromData.thick" style="width:30%" disabled></el-input>
          </el-form-item>
        </el-row>
        <el-row class="row-item">
        ...
        <el-row class="row-item">
          <el-form-item label="涂料防水层的平均厚度实测:">
            <el-select
              v-model="addFromData.thickness"
              placeholder="请选择"
              style="width:21%"
              @change="questionChange($event, 'thickness')"
            >
            ...
            <el-popover
              ...
              heigt="30"
              trigger="click"
              content="地下防水工程质量验收规范GB 50208-2011第4．4．8条涂料防水层的平均厚度应符合设计要求，最小厚度不得小于设计厚度的90％。"
            >
        ...
        <el-row class="row-item">
          <el-form-item label>
            <div style="float:left">
              <upload
                :isChecked="true"
                :fileList="thicknessFileList"
                @removefileList="fileRemove($event, 'thicknessFileList')"
              ></upload>
            </div>
          </el-form-item>
        </el-row>
        <el-row class="row-item">
          <el-form-item label="蓄/淋水试验:">
            <el-select v-model="addFromData.storage" placeholder="请选择" style="width:30%">
        ...
        <el-row class="row-item">
          <el-form-item label v-if="addFromData.leakage == '1'">
            <div style="float:left">
              <upload
                :isChecked="true"
        ...
        <el-row class="row-item">
          <el-form-item label="验收结果:">
            <el-select v-model="addFromData.result" placeholder="请选择" style="width:30%">
  ...
  methods: {
          ...
          if (this.$route.query.continue == 1) {
            res.data.data.publiList[0].bodyParam.forEach((item) => {
              if (item.key === "rainwater") {
                ...
                this.$set(this.addFromData, "thickness", item.comboxfile.value);
                this.thicknessFileList = item.comboxfile.fileList;
              }
            });
          }
        }
    ...
    async save() {
      this.$refs.addFromData.validate((valid) => {
        if (valid) {
          if (this.thicknessFileList.length === 0) {
            this.$message({ type: "error", message: "请上传厚度实测图片" });
            return false;
          } else if (this.storageFileList.length === 0) {
          ...
          param.thickness = {
            value: this.addFromData.thickness,
            fileList: this.thicknessFileList,
            id: 1,
          };
