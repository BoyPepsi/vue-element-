  ...
  <div>
    <div class="bigbox" style="margin-top:40px">
      <div class="title">
        ...
        <el-button
          class="btn"
          @click="goUpdate(header.mainId, header.parentId, '/gzy/coilGrassrootsTreatment/add')"
      ...
      <div class="item-select">
        ...
        <el-row
          class="row-item"
        >防水卷材品牌： {{ detailFromData.brand | accordingIdFindName(filterList) }}</el-row>
        <el-row
          class="row-item"
        >防水卷材型号： {{ detailFromData.model | accordingIdFindName(filterList) }}</el-row>
        <el-row
          class="row-item"
    ...
    <div class="bigbox" v-for="item in layerListData" :key="item.mainId">
      <div class="title">
        <div class="text">附加层、铺贴层</div>
        <el-button
          class="btn"
          @click="goUpdate(item.mainId, item.parentId, '/gzy/coilAdditionalLayer/add')"
          v-if="$route.query.isCheck == 1"
        >修改</el-button>
      </div>
    ...
    <div class="bigbox" v-for="item in acceptanceListData" :key="item.mainId">
      <div class="title">
        <div class="text">防水卷材验收</div>
        <el-button
          class="btn"
          @click="goUpdate(item.mainId, item.parentId, '/gzy/coilCoilAcceptance/add')"
          v-if="$route.query.isCheck == 1"
        >修改</el-button>
      </div>
      ...
      if (res.data.status === "0") {
        if (res.data.data.headParam && res.data.data.headParam.length != 0) {
          ...
          const bodyData = res.data.data.publiList.filter((item) => {
            return item.codeName === "附加层、铺贴层检查";
          });
