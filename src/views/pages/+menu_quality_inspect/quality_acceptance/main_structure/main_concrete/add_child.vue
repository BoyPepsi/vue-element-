<template>
  <div id="quality-inspect-save">
    <div class="item-select">
      ...
      <el-row class="row-item">墙柱楼层:{{formInfo.wallNum | accordingIdFindName(filterList)}}</el-row>
      <el-row class="row-item">梁板楼层:{{formInfo.beamNum | accordingIdFindName(filterList)}}</el-row>
    ...
    <div class="question-container">
      <addQuestionsNew code="040102020402" ref="addQuestion" :showOtherPostion="false"></addQuestionsNew>
    </div>
...
export default {
  ...
  created() {
    ...
    this.raiseList = JSON.parse(
      JSON.stringify(this.commonData.mainStructureRaiseList)
    );

    console.log(this.commonData.mainStructureRaiseList);
  },
  destroyed() {},
  methods: {
    /**
    ...
    async getChcekedData() {
      ...
      if (res.data.status === "0") {
        if (res.data.data && res.data.data.headParam.length > 0) {
          ...
          let date = res.data.data.headParam.find(
