  ...
  <div id="quality-inspect-save">
    <div class="wrap">
      <div v-for="(item) in checkList" :key="item.id">
        <div v-if="item.optionType != 3" class="list-item">
      ...
      <div class="qualified">
        检查意见:
        <span v-if="isQualified">合格</span>
        <span v-if="!isQualified">不合格</span>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  watch: {
     ...
     * date:2020-08-18
     */
    isQualified(newValue) {
      this.$emit("isQualified", newValue);
    },
    checkList: {
      handler(newValue) {
        let num = 0;
    ...
    checkedArr: {
      handler(newValue, oldValue) {
        // this.copyChecked = JSON.parse(JSON.stringify(newValue));
        if (newValue && newValue.length > 0) {
          this.checkList.forEach((item) => {
            newValue.forEach((ele) => {
              if (ele.checkId === item.id) {
                item.isChecked = true;
                if (ele.questionConten.length > 3) {
                  item.fileList = JSON.parse(ele.questionConten);
                } else {
                  item.fileList = [];
                }
                item.fileUrl = ele.checkPic;
              }
            });
          });
        }
      ...
      try {
        ...
        if (res.data.status === "0") {
          if (res.data.data.list.length > 0) {
            ...
            /*   this.checkList = res.data.data.filter(
              item => item.optionType !== 3
            ); */
          }
